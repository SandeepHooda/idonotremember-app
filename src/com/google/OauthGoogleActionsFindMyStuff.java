...
/**
 * Servlet implementation class OauthGoogleActionsFindMyStuff
 */
@WebServlet("/OauthGoogleActionsFindMyStuff")
public class OauthGoogleActionsFindMyStuff extends HttpServlet {
     ...
     */
    public OauthGoogleActionsFindMyStuff() {
        super();
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (null == client_id) {//Just a safety check - It don't happen
			client_id = OauthGoogleActionsFindMyStuff.client_id;
		...
		if (null != refresh_token) {//Step 3
		 ...
		 if (null != code) {//Step 2. get access token from auth code
			...
			if (null != state ) {
				...
				String loginVOStr = MangoDB.getDocumentWithQuery("google-find-stuff", "registered-users", code, "accessToken", false, null,null);
				Gson  json = new Gson();
	...
	private UserObj addCookiedToResponseAndRecordLoginInDB(HttpServletRequest request, HttpServletResponse response,  String access_token, String refreshToken) throws IOException {
		...
		addCookie("regID" , loginVO.getRegID(),request, response );
         ...
         MangoDB.createNewDocumentInCollection("google-find-stuff", "registered-users", data, null);
	...
	private void getAuthCode(HttpServletRequest request, HttpServletResponse response, String client_id, String state){
		//Client id + redirect url + scope + response type
			...
			String redirectUrl = "https://accounts.google.com/o/oauth2/auth?response_type=code&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.email%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.profile&client_id="+client_id+"&state="+state+"&redirect_uri=https%3A%2F%2Fidonotremember-app.appspot.com%2FOauthGoogleActionsFindMyStuff&output=embed";
			try {
	...
	private  Map<String,String> getAccesstokenFromRefreshToken(HttpServletRequest request, HttpServletResponse res, String refreshToken, String client_id) throws IOException{
		...
		String loginVOStr = MangoDB.getDocumentWithQuery("google-find-stuff", "registered-users", refreshToken, "refreshToken", false, null,null);
		Gson  json = new Gson();
		LoginVO loginVO = json.fromJson(loginVOStr, new TypeToken<LoginVO>() {}.getType());
		if (null != loginVO) {
	...
	private  Map<String,Object> getAccesstoken(HttpServletRequest request, HttpServletResponse res, String code, String client_id) throws IOException{
		...
		String urlParameters  = "grant_type=authorization_code&client_id="+client_id+"&client_secret="+client_secret+"&redirect_uri=https%3A%2F%2Fidonotremember-app.appspot.com%2FOauthGoogleActionsFindMyStuff&code="+code+"&output=embed";
	...
	public Map<String, String> getUserEmailFromMangoD(String accessToken) throws IOException{
		String loginVOStr = MangoDB.getDocumentWithQuery("google-find-stuff", "registered-users", accessToken, "accessToken", false, null,null);
		Gson  json = new Gson();
		LoginVO loginVO = json.fromJson(loginVOStr, new TypeToken<LoginVO>() {}.getType());
		Map<String, String> map = new HashMap<String, String>();
		if (null != loginVO) {
